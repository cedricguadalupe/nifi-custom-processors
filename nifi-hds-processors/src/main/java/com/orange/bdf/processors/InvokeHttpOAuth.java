...
import com.orange.bdf.processors.utils.ProxyAuthenticator;
import com.orange.bdf.processors.utils.SoftLimitBoundedByteArrayOutputStream;
...
@SupportsBatching
@Tags({"http", "https", "rest", "client", "oauth"})
...
public final class InvokeHttpOAuth extends AbstractProcessor {
    ...
    public static final PropertyDescriptor PROP_SSL_IGNORE = new PropertyDescriptor.Builder()
            .name("SSL Ignore")
            .description("This option allows curl to proceed and operate even for server connections otherwise considered insecure.")
            .required(false)
            .defaultValue("false")
            .allowableValues("true", "false")
            .build();
    ...
    public static final PropertyDescriptor PROP_OAUTH = new PropertyDescriptor.Builder()
            .name("OAuth authentification")
            .description("Enabling this property enabled OAuth authentification.")
            .required(false)
            .defaultValue("false")
            .allowableValues("true", "false")
            .build();

    public static final PropertyDescriptor PROP_OAUTH_BASE_URL = new PropertyDescriptor.Builder()
            .name("OAuth base url")
            .displayName("OAuth base url")
            .description("Url used to generate signature oauth")
            .required(false)
            .expressionLanguageSupported(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .build();

    public static final PropertyDescriptor PROP_OAUTH_CONSUMER_KEY = new PropertyDescriptor.Builder()
    ...
    public static final PropertyDescriptor PROP_OAUTH_CONSUMER_SECRET = new PropertyDescriptor.Builder()
    ...
    public static final PropertyDescriptor PROP_OAUTH_TOKEN_KEY = new PropertyDescriptor.Builder()
            .name("OAuth token key")
            .displayName("OAuth token key")
            .description("OAuth token key")
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .build();

    public static final PropertyDescriptor PROP_OAUTH_TOKEN_SECRET = new PropertyDescriptor.Builder()
    ...
    public static final PropertyDescriptor PROP_OAUTH_ARGUMENTS = new PropertyDescriptor.Builder()
            .name("OAuth arguments")
            .displayName("OAuth arguments")
            .description("OAuth arguments")
            .required(false)
            .expressionLanguageSupported(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .build();

    public static final PropertyDescriptor PROP_IOSW = new PropertyDescriptor.Builder()
            .name("IOSW")
            .description("Enabling this property enabled IOSW authentification.")
            .required(false)
            .defaultValue("none")
            .allowableValues("none", "basic", "header_custom")
            .build();
    ...
    public static final List<PropertyDescriptor> PROPERTIES = Collections.unmodifiableList(Arrays.asList(
            ...
            PROP_OAUTH_TOKEN_KEY,
            PROP_OAUTH_TOKEN_SECRET,
            PROP_OAUTH_ARGUMENTS,
            PROP_IOSW));
    ...
    public void setUpClient(final ProcessContext context) throws IOException, UnrecoverableKeyException, CertificateException, NoSuchAlgorithmException, KeyStoreException, KeyManagementException {
        ...
        if(!context.getProperty(PROP_SSL_IGNORE).asBoolean()) {
            ...
            okHttpClientBuilder.sslSocketFactory(sc.getSocketFactory(), (X509TrustManager) trustAllCerts[0]);
            okHttpClientBuilder.hostnameVerifier(allHostsValid);
    ...
    private String getOAuthAuthorization(final ProcessContext context, final FlowFile requestFlowFile) throws UnsupportedEncodingException, SignatureException {
        final String method = context.getProperty(PROP_METHOD).getValue();
        final String arguments = context.getProperty(PROP_OAUTH_ARGUMENTS).evaluateAttributeExpressions(requestFlowFile).getValue();
        final String consumerKey = context.getProperty(PROP_OAUTH_CONSUMER_KEY).getValue();
        final String consumerSecret = context.getProperty(PROP_OAUTH_CONSUMER_SECRET).getValue();
        final String tokenKey = context.getProperty(PROP_OAUTH_TOKEN_KEY).getValue();
        final String tokenSecret = context.getProperty(PROP_OAUTH_TOKEN_SECRET).getValue();
        final String signatureMethod = "HMAC-SHA1";
        final String baseUrl = context.getProperty(PROP_OAUTH_BASE_URL).evaluateAttributeExpressions(requestFlowFile).getValue();
        ...
        String oauth_timestamp = Long.toString(ts.getTime()/1000);
        map.put("oauth_consumer_key", consumerKey);
        map.put("oauth_nonce", oauth_nonce);
        map.put("oauth_signature_method", signatureMethod);
        map.put("oauth_timestamp", oauth_timestamp);
        map.put("oauth_token", tokenKey);
        map.put("oauth_version", "1.0");

        String parameterString = "";
        for(String key: map.keySet())  {
            parameterString += encode(key);
            parameterString += '=';
            parameterString += encode(map.get(key));
            parameterString += '&';
        }
        ...
        signatureBaseString += '&';
        signatureBaseString += encode(parameterString);
        ...
        oauth += encode(consumerKey);
        oauth += "\", ";
        oauth += "oauth_nonce=\"";
        oauth += encode(oauth_nonce);
        oauth += "\", ";
        oauth += "oauth_signature=\"";
        oauth += encode(oauthSignature);
        oauth += "\", ";
        oauth += "oauth_signature_method=\"";
        oauth += encode(signatureMethod);
        oauth += "\", ";
        oauth += "oauth_timestamp=\"";
        oauth += encode(oauth_timestamp);
        oauth += "\", ";
        oauth += "oauth_token=\"";
        oauth += encode(tokenKey);
        oauth += "\", ";
        oauth += "oauth_version=\"";
        oauth += encode("1.0");
        oauth += "\"";
    ...
    private static String encode(String str) throws UnsupportedEncodingException {
        return java.net.URLEncoder.encode(str, "UTF-8").replace("+", "%20").replace("*", "%2A").replace("%7E", "~");
    }
    ...
    private Request configureRequest(final ProcessContext context, final ProcessSession session, final FlowFile requestFlowFile, URL url) {
        ...
        if (!authUser.isEmpty() && "false".equalsIgnoreCase(context.getProperty(PROP_DIGEST_AUTH).getValue())) {
            ...
            final String iosw_mode = context.getProperty(PROP_IOSW).getValue();
            if("header_custom".equals(iosw_mode)) {
                requestBuilder = requestBuilder.header("X-IOSW-AUTH", Base64.getEncoder().encodeToString((authUser + ":" + authPass).getBytes()));
            } else {
                requestBuilder = requestBuilder.header("Authorization", credential);
            }
        }

        if(context.getProperty(PROP_OAUTH).asBoolean()) {
            try {
                requestBuilder = requestBuilder.header("Authorization", getOAuthAuthorization(context, requestFlowFile));
            } catch (UnsupportedEncodingException e) {
                getLogger().error("ERROR - OAuth Problem 1");
                e.printStackTrace();
            } catch (SignatureException e) {
                getLogger().error("ERROR - OAuth Problem 2");
                e.printStackTrace();
            }
        }
